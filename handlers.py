...
async def callback_reassign(call: types.CallbackQuery):
...
def register_game_handlers(dp: Dispatcher):
    dp.register_callback_query_handler(callback_assign, lambda c: c.data == 'commit_ships')
    dp.register_callback_query_handler(callback_reassign, lambda c: c.data == 'reassign_ships')
    dp.register_message_handler(handle_play, commands=['play'])
    dp.register_message_handler(handle_rules, commands=['rules'])
    dp.register_message_handler(handle_info, commands=['info'])
    dp.register_message_handler(handle_start, commands=['start'])
    dp.register_message_handler(assign_ships_start, commands=['assign'])
    dp.register_message_handler(assign_ships_1_deck, state=FSMShips.ships_1_deck)
